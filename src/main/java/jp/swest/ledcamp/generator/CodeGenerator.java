...
import jp.swest.ledcamp.generator.FileUploader;
...
public class CodeGenerator {
  ...
  public static class ConflictCheckVisitor extends SimpleFileVisitor<Path> {
    ...
    public FileVisitResult visitFile(final Path file, final BasicFileAttributes attrs) throws IOException {
      ...
      Path _parent = targetFile.getParent();
      boolean _exists = Files.exists(_parent);
      boolean _not = (!_exists);
      if (_not) {
        Path _parent_1 = targetFile.getParent();
        Files.createDirectories(_parent_1);
      }
    ...
    public FileVisitResult preVisitDirectory(final Path dir, final BasicFileAttributes attrs) throws IOException {
      boolean _exists = Files.exists(dir);
      boolean _not = (!_exists);
      if (_not) {
  ...
  private final static String uploadURL = "http://mdd-compile.shinshu-u.ac.jp/upload";
  ...
  public static void transferToCompilerServer() {
    try {
      ...
      File _file_2 = zipPath.toFile();
      final String dlURL = FileUploader.fileUpload(uploadURL, _file_2);
      Thread.sleep(3000);
      String _targetPath_1 = setting.getTargetPath();
      Path _get = Paths.get(_targetPath_1);
      FileUploader.fileDownload(dlURL, _get);
